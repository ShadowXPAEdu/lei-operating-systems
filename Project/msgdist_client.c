		...
		if (IsServerRunning(sv_fifo) == 0) {
			...
			if (mkfifo(cl_cfg.cl_fifo, 0666) == 0 && mkfifo(cl_cfg.cl_fifo_tt, 0666) == 0) {
				...
				if (cl_cfg.cl_fifo_fd == -1 || cl_cfg.cl_fifo_tt_fd == -1) {
					...
					if (pthread_create(&tds[1], NULL, heartbeat, NULL) != 0) {
						cl_exit(-2);
					}
...
void *handle_connection(void *p_cmd) {
	...
	if (r_cmd.cmd != CMD_HEARTBEAT && r_cmd.cmd != CMD_ALIVE && r_cmd.cmd != CMD_IGN) {
		char str[31];
...
void *heartbeat() {
	COMMAND s_cmd;
	s_cmd.cmd = CMD_HEARTBEAT;
	snprintf(s_cmd.From, MAX_FIFO, "%s", cl_cfg.cl_fifo);
	while (cmd_heartbeat == 0) {
		pthread_mutex_lock(&mtx_hb);
		cl_cfg.sv_offline = 1;
		pthread_mutex_unlock(&mtx_hb);
		if (cmd_heartbeat == 0)
			write(cl_cfg.sv_fifo_fd, &s_cmd, sizeof(COMMAND));
		sleep(10);
		pthread_mutex_lock(&mtx_hb);
		if (cl_cfg.sv_offline) {
			sv_shutdown = 1;
			shutdown();
			pthread_mutex_unlock(&mtx_hb);
			return NULL;
...
void shutdown() {
	if (shutdown_init == 0) {
		...
		printf("Please wait while the client closes.\n");
	}
}
